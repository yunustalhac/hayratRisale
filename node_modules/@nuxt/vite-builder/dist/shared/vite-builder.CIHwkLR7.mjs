...
import { dirname, normalize, resolve, join, isAbsolute, relative } from 'pathe';
...
import { createApp, toNodeListener, defineEventHandler, createError, handleCors, setHeader } from 'h3';
...
function registerSSRHandlers(app, ssrServer, clientServer) {
  ...
  app.use("/resolve", defineEventHandler(async (event) => {
    ...
    return await ssrServer.pluginContainer.resolveId(decodeURIComponent(id), importer ? decodeURIComponent(importer) : void 0).catch(() => null);
  }));
...
async function initViteNodeServer(nuxt, ssrServer) {
